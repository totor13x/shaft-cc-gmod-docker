...
if SERVER then
	function common.shoot(ply,ent,tbl)
		...
		if ent.formTable.canAim&&!ent.formTable.canAim(ply,ent) then return end
		if aim.usesSecondaryEnt&&!IsValid(ent:GetPillAimEnt()) then return end

		local aimEnt = aim.usesSecondaryEnt&&ent:GetPillAimEnt()||(ent.formTable.type=="ply"&&ent:GetPuppet())||ent
		...
		elseif aim.offset then
			...
			a.p = 0
			start= {Pos=ply:EyePos()+a:Forward()*aim.offset,Ang=ply:EyeAngles()}
		else
			return
		end
		...
		bullet.Tracer      = tbl.tracer && !aim.fixTracers && 1 || 0
