...
function setAiTeam(ent,side)
	if ent:IsPlayer()&&side=="default" then
		team_map[ent]=nil
	else
		team_map[ent]=side
	end
	...
	for _,npc in pairs(ents.FindByClass("npc_*")) do
		if npc==ent or !npc.AddEntityRelationship or !IsValid(npc) then continue end
		...
		elseif side=="wild" then
			...
			local relationship = (side_relationships[otherSide]||{})[side]
		...
		if ent:IsNPC() then
			...
			elseif side=="wild" then
				...
				local relationship = (side_relationships[side]||{})[otherSide]
				if relationship then
	...
	if !ent:IsPlayer() and ent:GetTable() then
		ent:CallOnRemove("ClearAiTeam",function(ent) team_map[ent]=nil end)
	end
end
...
hook.Add("OnEntityCreated","pk_pill_npc_spawn",function(npc)
	if npc.AddEntityRelationship then
		...
		for ent,side in pairs(team_map) do
			if !IsValid(ent) then continue end //This really shouldn't happen. But it does. ):
			...
			if ent:IsNPC() then //TODO CHECK AGAINST ALL NPCS, NOT JUST ONES IN THIS LIST -- THIS MIGHT ACTUALLY BE RIGHT
				if otherSide==side or otherSide=="harmless" then
					ent:AddEntityRelationship(npc, D_LI, 99)
				elseif side=="wild" then
					ent:AddEntityRelationship(npc, D_HT, 99)
