...
function ENT:Initialize()
	...
	if SERVER then
		...
		pk_pills.setAiTeam(self,self.formTable.side||"default")
...
function ENT:PhysicsUpdate()
	if SERVER && self.drive.functions then
...
function ENT:OnRemove()
	...
	//local n= pk_pills.unmapEnt(self:GetPillUser(),self)

	if !pk_pills.unmapEnt(self:GetPillUser(),self) then
		if SERVER then
...
if CLIENT then
	...
	function ENT:Think()
		...
		if self.formTable.pose then
			for k,f in pairs(self.formTable.pose) do
				...
				if new then self:SetPoseParameter(k,new) end
			end
		end

		if self.formTable.aim&&!self.formTable.aim.usesSecondaryEnt||IsValid(self:GetPillAimEnt()) then
			local aimEnt = self.formTable.aim.usesSecondaryEnt&&self:GetPillAimEnt()||self
			if self.formTable.aim.xPose && self.formTable.aim.yPose then
				if !self.formTable.canAim||self.formTable.canAim(ply,self) then
		...
		if (self.formTable.damageFromWater && self:WaterLevel() > 1) then
		...
		if ply:KeyDown(IN_ATTACK) && self.formTable.attack && self.formTable.attack.mode=="auto" then
			...
			if (!self.lastAttack || self.formTable.attack.delay<CurTime()-self.lastAttack) then
		...
		if ply:KeyDown(IN_ATTACK2) && self.formTable.attack2 && self.formTable.attack2.mode=="auto" then
			...
			if !self.lastAttack2 || self.formTable.attack2.delay<CurTime()-self.lastAttack2 then
	...
	function ENT:StartTouch(TouchEnt)
		if !IsValid(TouchEnt:GetPhysicsObject()) then return end
		if (TouchEnt:IsPlayer() || TouchEnt:IsNPC() || TouchEnt:GetClass() == "pill_ent_phys") && TouchEnt:GetPhysicsObject():GetMaterial() != "metal" then
	...
	function ENT:OnTakeDamage(dmg)
		if (self.formTable.diesOnExplode && dmg:GetDamageType() == DMG_BLAST) then
			self:PillDie()
		end

		if self.formTable.health then
			if !self.formTable.onlyTakesExplosiveDamage then
...
function ENT:PillSound(name,bulk) //The "bulk" parameter should only be used if you plan to play a ton of sounds in quick succession.
	...
	if isstring(s) then
		if bulk then
			sound.Play(s,self:GetPos(),self.formTable.sounds[name.."_level"]||100,self.formTable.sounds[name.."_pitch"]||100,1)
		else
			self:EmitSound(s,self.formTable.sounds[name.."_level"]||100,self.formTable.sounds[name.."_pitch"]||100)
		end
		return true
	end
end
...
function ENT:PillLoopStopAll()
	if !self.loopingSounds then return end
